...
func (apiCfg *apiConfig) handlerFeedFollowGetByUser(w http.ResponseWriter, r *http.Request, user database.User) {
	feedFollows, err := apiCfg.DB.GetFeedFollowsByUser(r.Context())
	if err != nil {
...
func (apiCfg *apiConfig) handlerFeedFollowCreate(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	if len(params.FeedID) == 0 {
		respondWithError(w, http.StatusBadRequest, "feed id cannot be empty")
		return
	}
